...
Serial.println("Access Granted!");
  ...
  if(j==0){
    ...
    Serial.printf("Set state... %s\n", Firebase.RTDB.setInt(&fbdo, "/A/state", true) ? "ok" : fbdo.errorReason().c_str());

  
    // Serial.printf("Set credit... %s\n", Firebase.RTDB.setInt(&fbdo, "/A/credit", 15000) ? "ok" : fbdo.errorReason().c_str());

   // Serial.printf("Get credit... %s\n", Firebase.RTDB.getInt(&fbdo, "/A/credit") ? String(fbdo.intData()).c_str() : fbdo.errorReason().c_str());
    }

if(j==1){
    Serial.printf("Set state... %s\n", Firebase.RTDB.setInt(&fbdo, "/B/state", true) ? "ok" : fbdo.errorReason().c_str());
...
if(j==2){
    Serial.printf("Set state... %s\n", Firebase.RTDB.setInt(&fbdo, "/C/state", true) ? "ok" : fbdo.errorReason().c_str());
